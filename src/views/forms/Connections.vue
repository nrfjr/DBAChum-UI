...
import { upsertConnectionToAPI, getAvailableDB, getExistingConnections, deleteConnection } from '../../assets/js/settings.js'
...
export default {
    ...
    methods: {
        upsertConnection: async function () {
            ...
            let result = await upsertConnectionToAPI(this.selectedDB, JSON.stringify(this.formData));
        ...
        deleteConnectionDetails: async function () {
            ...
            let result = await deleteConnection(this.selectedDB, this.selectedId);
            this.spinner.show = false
            if (result.data.status) {
