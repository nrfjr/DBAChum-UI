...
import { upsertDiskServerToAPI, getExistingDiskServers, deleteDiskServer } from '../../assets/js/settings.js'
...
export default {
    ...
    methods: {
        upsertDiskServer: async function () {
            ...
            let result = await upsertDiskServerToAPI(this.selectedType, JSON.stringify(this.formData));
        ...
        deleteDiskServerDetails: async function () {
            ...
            let result = await deleteDiskServer(this.selectedType, this.selectedId);
            this.spinner.show = false
            if (result.data.status) {
